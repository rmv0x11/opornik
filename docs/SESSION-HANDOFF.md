...
- **Следующие рычаги (не сделаны):** (а) фичи блокировки/прайма в кодировке + переобучение с нуля; (б) AlphaZero (policy+value+MCTS); (в) реальные партии LogasAI → `agree` → точечно. Идёт исследование «как сделать мощнейший движок» (возможно, переписывание с нуля).
