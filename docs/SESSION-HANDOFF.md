...
**▶ ПОЛНАЯ СКОРОСТЬ (пользователь снял 6-ядерный лимит ~21:40; throttler убит): gen-3 data → ~02:40, вердикт gen-3 → ~04:30. План: вердикт gen-3 → блок-сьют этапа 3 → WASM-интеграция v2 + деплой → ПАУЗА (просьба пользователя: после закрытия этапа остановиться). Прод-гейт дуэль (v2@2ply vs v1@3ply) оказалась 5–10 ч (3-ply в engine-train без leaf-бюджета, ~1–2 мин/партия) — решает только ветку этапа 4. TODO тулинга: прогресс-вывод в duel + leaf-бюджет для честного прод-сравнения. Процессы:**
