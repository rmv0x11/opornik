...
**Этап 2 начат:** er/duel/bench принимают NV2P-пары (`AnyEval`, сниффинг magic; `er --load` скорит любой формат, генерация меток требует v1-politику; смоук: v1 воспроизводит базу 0.0557/0.0563 бит-в-бит). **Осталось в этапе 2:** дождаться diag2-big (фон, `/tmp/opornik-diag2-big.log`) → ночной dataset 10k×432 `--exclude data/evalset-v1.tsv` (~3 ч, ПОСЛЕ diag2 — CPU) → encode → train (см. train/README.md) → duel vs чемпион + er --load; гейт: mEMG падает по поколениям, дуэль ≥55% значимо.
