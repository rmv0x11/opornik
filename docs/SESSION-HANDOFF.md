...
**В фоне (проверить логи!):** (1) `/tmp/opornik-gen2data.log` — gen-2 data 80k×216 seed 1313 (~5 ч); (2) `/tmp/opornik-distill.log` — дистилляция: 300k позиций с метками-вероятностями чемпиона (`dataset --label net`, новая команда) → train 100 эпох → er/duel; если дистиллят ≈ чемпион по ER — он станет `--init` для обучения gen-2 (старт с уровня чемпиона вместо нуля).
**Следующее по готовности:** gen-2 train на gen1+gen2 (~90k строк, warm-start от лучшего из {distill, gen1}) → duel/er; гейт этапа 2: ER на evalset падает по поколениям, дуэль ≥55% значимо. Race-фаза рецептом уже подтверждена.
