...
**▶ ИДЁТ с лимитом 6 ядер (пользователь, ~21:00, термоконтроль): gen-3 dataset дросселируется duty-циклом до ~5 ядер (фоновый throttler `b9o0kbgfv`, лог `/tmp/opornik-throttle.log`; убить throttler = вернуть полную скорость), готовность сдвинулась к ~06:30. ВСЕ новые тяжёлые запуски — с `--threads 6` (флаг добавлен в dataset/er/diag2, `c108337`). Процессы:**
