...
## ⏸ ЭТАП 0 ДВИЖКА V2 — ревью пройдено, идут перезамеры (2026-06-11)
Контекст: «поехали» по `docs/engine-v2-plan.md` (читать первым). Этап 0 (тулинг, `41e0fbc`) прошёл adversarial-ревью (45 агентов): **11 подтверждённых дефектов исправлены** — полный список в `docs/training-log.md` («Этап 0 … ревью», 2026-06-11). Ключевое: терминальный баг diag2 (флип знака у выигрышей Чёрных!), mars-слепое усечение truth-меток, выборка «200 позиций = 2 игры», шумовой пол в вердикте diag2, по-позиционный RNG (независимость от числа ядер), точный round-trip --save/--load, `dataset --exclude` против утечки eval↔train, строгий CLI + `# meta:` provenance. 15/15 тестов (5 новых), смоуки: diag2 / er save=load бит-в-бит / dataset --exclude.
**В фоне крутится (проверить!):** `/tmp/opornik-stage0-v2.log` — перегенерация `data/evalset-v1.tsv` (er: 200 поз., trials 120, seed 77, **--stratify 25**, --per-phase) → исправленный diag2 (72 поз., truth/floor 128, leaf 24, seed 99), суммарно ~3–4 ч. Если оборвано — перезапустить эти две команды из лога. **Старые числа 2026-06-10 (ER 0.0729/0.0672, mars-слепые) невалидны** — не сравнивать с новыми; уже валидный вывод: ошибка сети сконцентрирована в contact-фазе (ER 0.0837, agree 17.9%).
**Сделано дальше (2026-06-11):** eval-set v1 закоммичен (`data/evalset-v1.tsv`, 249 решений, база чемпиона ER 0.0557/0.0563, contact 0.0776 — см. training-log). diag2 №1 (128 trials) — inconclusive: шумовой пол выше всех чузеров; в diag2 добавлена парная статистика (er2−erl ± SE, z); **большой diag2 крутится в фоне** (`/tmp/opornik-diag2-big.log`, 200 поз., truth/floor 256, leaf 64, ~4 ч) — интерпретировать по z парной разницы (z≥2 ⇒ ошибки листьев систематические ⇒ рецепт v2 подтверждён) и вписать в training-log.
**ЭТАП 1 РЕАЛИЗОВАН (`9c012b7`):** `engine-core`: `phase.rs` (has_contact в ядре), `encoding2.rs` (contact 217 входов = raw с фиксом `==3`→`>=3` + 21 экспертная фича: блоки во ВРАЖЕСКОМ фрейме (линейные пробеги, маппинг (f+11)%24+1), запертые шашки+глубина, голова/запас/стэки/дом/pip-lead/free-pips-тайминг; race 196 = только raw), `net2.rs` (NetV2 multi-layer ReLU+4-softmax, PhaseNets c диспетчером has_contact, формат NNV2/NV2P). `engine-train`: `encode` (TSV→NNF2 фичи, роутинг = боевой диспетчер), `netbench` (v1 7.3 µs vs v2 88 µs = ×12.1 — в бюджете плана). `train/train_v2.py` (PyTorch, CE на soft-labels, MPS, экспорт NV2P; probe сверен с Rust до 1e-6; venv: `/opt/homebrew/bin/python3.14 -m venv train/.venv` — у 3.15 нет колёс torch). Конвейер по шагам — `train/README.md`. **Ревью этапа 1 ПРОЙДЕНО, 7 дефектов исправлены (`c7deb40`)**: [major] spares дважды вычитал голову при голове внутри блока (~50% contact-позиций!), гейт block-фич на len≥2, depth/23, free_pips = один держатель на клетку, u64-чек длины в from_bytes (wasm32), тренер: ранний выход на пустом файле + честный --val 0. 86 тестов.
**Этап 2 идёт.** er/duel/bench принимают NV2P (`AnyEval`); diag2-big: rollout-leaf лучше net-2ply на +0.0066±0.0042 (z=1.56, направленно — да, значимо — нет; решение: поколения отвечают прямо). **Gen-1 (10k×432, 78 мин!): проигрыш чемпиону** (дуэль 19.2%/15.0%, ER 0.0764 vs 0.0557), НО bearoff 0.0011 УЖЕ ЛУЧШЕ чемпиона (0.0045) на ~1k строк; contact недокормлен (8904 строки, val CE плато 0.716). Сеть: `models/nardy-v2-gen1.bin` (локально, в git не идёт — política /models/*). Подробности в training-log.
**ЭТАП 2 ВЗЯТ ПОЛНОСТЬЮ (2026-06-11): gen-2-warm ПОБЕДИЛ чемпиона на ОБЕИХ глубинах.** Рецепт: дистилляция чемпиона в v2 (300k меток-вероятностей, 8 мин, `dataset --label net`) → fine-tune на 90k роллаут-меток (gen1+gen2, lr 3e-4). Дуэли: **1-ply 55.5% (222/400, p=0.028); 2-ply 56.0% (672/1200, z=4.16, p≈3e-5)**. ER 2-ply 0.0484 — лучший в проекте. Контроль с нуля на тех же данных: 0.0737 — **дистилляция решающая**. Векторизация: v2 88→13.6 µs, v1 6.5→1.8 µs (`dot` 8 линий, wasm-гейт). **Этап 3 (качественно) пройден: v2 видит праймы статически, v1 прайм-слеп** (блок-тест в training-log). Подробности — training-log.
**Чемпион ПЕТЛИ = `models/nardy-v2-champion.bin`** (копия gen2-warm). Прод (браузер) НЕ менялся — для смены нужны уверенное 2-ply превосходство + WASM-интеграция PhaseNets (этап 4: загрузка NV2P-ассета, диспетчер, top-4 root pruning).
**✅ ЭТАП ЗАКРЫТ ЦЕЛИКОМ (2026-06-12 ~05:50) — ДВИЖОК V2 ЗАДЕПЛОЕН (v0.9.23, gh-pages `dc4e12e`, бандл `index-BzM9l4EC.js`, live 2/2).**

## Сессия 2026-06-12 (день): v0.9.27 PWA + v0.9.28 клики
- **v0.9.27 — PWA «приложение для всех платформ»** (gh-pages `ea38f78`): vite-plugin-pwa (`registerType: 'prompt'` — обновление НЕ перезагружает партию, тост в `main.ts`), манифест ru/standalone, service worker прекэширует app shell + WASM + веса сети (~2.2 МБ, офлайн-игра целиком). Иконки рисуются из `web/icons-src/icon.svg` → `node icons-src/render-icons.mjs` (Playwright-рендер в `public/icons/`); офлайн-смоук: `node icons-src/pwa-smoke.mjs` при поднятом `vite preview --port 4173`.
- **v0.9.28 — клики по шашкам чинились от дрожания** (gh-pages `dc7d327`, бандл `index-DnHvceJY.js`, live 2/2): клик с джиттером >6px превращался в драг, драг выбирал шашку, хвостовой `click` снимал выбор → «ходить кликами нельзя» на телефонах/живой мыши. Фикс в `Board.svelte`: squelch хвостового click после драга, порог драга 8px (touch slop), новый проп `onDragStart` — драг принудительно выбирает тянутую шашку (раньше через `onPointClick` мог сыграть ход НА её клетку). Регрессионный e2e «клик с дрожанием». 34 e2e + preview + live зелёные.
Итоги ночи: **gen-3 побил gen-2** (1-ply 56.5% p=0.009; 60.3% против v1) — чемпион петли `models/nardy-v2-champion.bin` = gen-3 (в git). **Прод-гейт: v2@2ply > v1@3ply 56.3% (169/300, p=0.028)** (играл gen-2-веса; gen-3 сильнее). Этап 3: два блок-теста (v2 статически видит праймы; играет «перекат прайма», v1 ломает середину). **Этап 4 сделан**: `engine-wasm` NetKind V1/V2 + `withV2Bytes`/`netGeneration`, клампинг глубины v2→2-ply (сильнее И быстрее старого 3-ply), worker фетчит `nardy-v2-gen3.bin` (1.5 МБ, public/) с тихим фолбэком на встроенную v1; svelte-check 0, 33 e2e + preview + live зелёные. В проде сейчас v0.9.23.
**Следующее (после паузы), по выбору пользователя:** (а) продолжать петлю gen-4+ (датасет политикой gen-3, ночь; eval-set-v2 сгенерировать заодно — старый теряет смысл вне v1-политики); (б) top-4 root pruning + wasm-simd128 → дешёвый 3-ply для v2; (в) UI/анимации из исходного плана overhaul; (г) прогресс-вывод в duel + leaf-бюджет в engine-train (урок 12-часовой дуэли). TODO мелочь: `.github/workflows/deploy.yml` так и untracked (нужен ручной пуш со scope workflow).
1. **gen-3 data** (PID 41979, прошло ~4.2 ч из ~11): 80k×216 политикой нового чемпиона → `data/gen3.tsv`, лог `/tmp/opornik-gen3data.log`;
2. **прод-гейт дуэль** (PID 45967, ~2 ч): v2@2-ply vs v1@3-ply, 300 игр → `/tmp/opornik-prodgate.log` (решает, можно ли ставить v2 на сайт без top-4 pruning).
**ВОЗОБНОВИТЬ:** `pkill -CONT -f "target/release/engine-train" && pkill -CONT -f "opornik-gen3data\|opornik-prodgate"` (или `kill -CONT 41979 45967 41975 45945`). ⚠️ Перезагрузка/глубокий сон Мака убьёт замороженные процессы — тогда перезапустить команды из заголовков логов (потеря только времени, сиды детерминированы). После gen-3 data: encode gen1+2+3 (~170k строк) → train `--init models/nardy-v2-gen2-warm.bin --lr 3e-4 --epochs 150` → er --load + дуэли vs оба чемпиона (полная цепочка-образец в `/tmp/opornik-gen2train.log` заголовке).
Добор 2-ply дуэли ЗАВЕРШЁН: суммарно 672/1200 = 56.0%, p≈3e-5.
...
Версионирование: `web/package.json` → `__APP_VERSION__` (подвал меню) + `CHANGELOG.md` + версия в коммите ветки `gh-pages`. **В проде сейчас — v0.9.31** (`index-nafhoEVO.js`, gh-pages `df978a4`; live-смоук зелёный 2026-06-16) = исходникам, недокаченного нет.

## Сессия 2026-06-16: v0.9.31 — лист партии наоборот + тогл подсветки
- **Лист партии новейшим ходом вверх** (обратный порядок, как панель бросков): дериватив `historyView = history.map((h,i)=>({h,i})).reverse()` в `Play.svelte` сохраняет ИСХОДНЫЙ индекс, поэтому разбор/переигровка/просмотр позиции адресуют верную строку; прыжок-скролл `openReview` переведён с `:nth-child` на `li[data-idx]` (порядко-независимо).
- **Тогл «💡 подсказки»** (`.auto-toggle`, pref `opornik.hints`, по умолчанию ON) скрывает подсветку ходов на доске. В `Board.svelte` новый проп `showHints` гейтит ТОЛЬКО визуал (`class:source`, `.dot`); множества `sources`/`dests` по-прежнему питают клик и драг — игра полностью рабочая с выключенными подсказками. `selected`/`last`-пульс не трогаем (это обратная связь, не подсказка).
- Регрессионные e2e: обратный порядок листа; подсказки-офф прячут подсветку, но выбор/ход регистрируются, и тогл-он их возвращает. 38 e2e + svelte-check 0 + live-смоук зелёные.
