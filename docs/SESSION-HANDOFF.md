...
**ЭТАП 2 ВЗЯТ (2026-06-11 ~14:00): gen-2-warm ПОБЕДИЛ чемпиона.** Рецепт: дистилляция чемпиона в v2 (300k меток-вероятностей, 8 мин, `dataset --label net`) → fine-tune на 90k роллаут-меток (gen1+gen2, lr 3e-4). Дуэли: **1-ply 55.5% (222/400, p=0.028 — ЗНАЧИМО)**; 2-ply 53.5% (321/600, p≈0.086, добор до 1200 фоном). ER 2-ply 0.0484 — лучший в проекте. Контроль с нуля на тех же данных: 0.0737 — **дистилляция решающая**. Векторизация: v2 88→13.6 µs, v1 6.5→1.8 µs (`dot` 8 линий, wasm-гейт). Подробности — training-log.
